...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    float doubleJumpThrust = 8f;

    [SerializeField]
    float maxDoubleJumpHeight = 40f;
    bool grounded = true;
    ...
    [SerializeField]
    private LayerMask obstacleMask; // "Walls"
    ...
    [SerializeField]
    private float dropThroughDuration = 0.5f;
    ...
    void Awake()
    {
        ...
        if (characterA == null && transform.childCount > 0)
            characterA = transform.GetChild(0).gameObject;
        if (characterB == null && transform.childCount > 1)
    ...
    public void OnMove(InputAction.CallbackContext context)
    {
        ...
        if (context.performed)
            isMoving = Mathf.Abs(movementInput.x) > 0.01f;
        if (context.canceled)
        {
            isMoving = false;
            movementInput = Vector2.zero;
    ...
    IEnumerator SwitchCharacter()
    {
        ...
        if (activeAnimator != null)
            activeAnimator.SetTrigger("isSwitching");
    ...
    void TryStartRoll()
    {
        if (!grounded || isCrouching || isRolling)
            return;
    ...
    IEnumerator RollCoroutine(float dir)
    {
        ...
        float elapsed = 0f;
        if (activeAnimator != null)
            activeAnimator.SetTrigger("Rolling");
    ...
    void StartCrouch()
    {
        if (isCrouching || !grounded || isRolling)
            return;
        isCrouching = true;
        Vector2 newSize = new(
            originalCapsuleSize.x,
            originalCapsuleSize.y * CROUCH_HEIGHT_MULTIPLIER
        );
    ...
    void StopCrouch()
    {
        ...
        RaycastHit2D hitLeft = Physics2D.Raycast(
            originCenter + Vector2.left * halfWidth,
            Vector2.up,
            headroomNeeded,
            obstacleMask
        );
        ...
        RaycastHit2D hitRight = Physics2D.Raycast(
            originCenter + Vector2.right * halfWidth,
            Vector2.up,
            headroomNeeded,
            obstacleMask
        );
        ...
        {
            Debug.Log(
                $"Cannot stand: hit '{hitCenter.collider?.name}' at distance {hitCenter.distance}/{headroomNeeded}"
            );
        ...
        isCrouching = false;
        if (activeAnimator != null)
            activeAnimator.SetBool("Crouching", false);
    ...
    void Jump()
    {
        ...
        canDoubleJump = true;
        if (activeAnimator != null)
            activeAnimator.SetTrigger("Jumping");
    }
    ...
    void DoubleJump()
    {
        ...
        if (activeAnimator != null)
            activeAnimator.SetTrigger("DoubleJumping");
        canDoubleJump = false;
    }
    ...
    void Move()
    {
        ...
        if (movementInput.x > 0.01f && activeSR != null)
            activeSR.flipX = false;
        if (movementInput.x < -0.01f && activeSR != null)
            activeSR.flipX = true;
    }
